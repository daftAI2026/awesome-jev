...
- On desktop, GitHub use categories occupy a left rail beside the main results. On smaller screens the same navigation opens a left Sheet. All projects, Top 100 by stars, Jev news, and Saved are separate shortcuts above the category list, in that order; Top 100 uses the global star rank, not the current search or sort order. Saved sits directly below Jev news and its count comes from this browser's storage. Other counts come from their respective data stores.
