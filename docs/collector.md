...
- **Reports:** show UTC time, pinned README evidence, stars, the model's relevance/decision/confidence, and a conservative recommendation. Missing context is distinct from a low model score; HTTP failures are not rejections. Do not invent detailed defect explanations from confidence alone. Human assessment can consider other evidence; this bot is not a runtime or security certification.
