...
- On desktop, navigation occupies a left rail beside the main results. On smaller screens the same navigation opens a left Sheet. Top 100 by stars, Jev news, and Saved are the three shortcuts above the divider. All projects begins the project category group below it, followed by the individual categories. Top 100 uses the global star rank, not the current search or sort order. Saved sits directly below Jev news and its count comes from this browser's storage. Other counts come from their respective data stores.
...
Saved is a browser-local reading list, not a repository data source or an account. The one `SaveButton` component appears on project cards, list rows, news cards, and both previews. It uses Phosphor's `Bookmark` rather than a star, so GitHub stars retain their original meaning. The button is a sibling of the card's full-surface link, never nested inside it: activating it must neither navigate nor open the preview. On project cards the icon aligns with the title row's right content edge while rank lives in the metadata line; on news cards it aligns with the source row's right content edge without crowding source or score. List rows align it to the row's right content edge. Cards and rows use a visually compact 32px control with an invisible 40px pointer target; previews keep a 40px control beside Close. Visible focus, `aria-pressed`, and distinct add/remove labels make the state usable without relying on color alone.
...
GitHub cards show title, summary, source repository / language, recorded stars / forks / issues, and tags when available. Star rank sits quietly in the metadata line, leaving the top-right corner for Save. Counts and sort keys come from `sourceMeta`; never substitute invented values. Peer cards keep the same type roles and metadata order even when summaries have unequal lengths.
