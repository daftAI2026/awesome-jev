...
Saved is a browser-local reading list, not a repository data source or an account. The one `SaveButton` component appears on project cards, list rows, news cards, and both previews. It uses a bookmark rather than a star, so GitHub stars retain their original meaning. The button is a sibling of the card's full-surface link, never nested inside it: activating it must neither navigate nor open the preview. On project cards it owns the upper-right corner while rank moves into the metadata line; on news cards it occupies the upper-right corner while source and score remain readable. Both previews place it beside Close. A 40px target, visible focus, `aria-pressed`, and distinct add/remove labels make the state usable without relying on color alone.
