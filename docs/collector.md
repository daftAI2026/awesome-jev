...
- **Budget:** at most 10 submitted projects per request run, 100 new-project review slots per UTC day across this bot, and a 10-minute per-request cooldown. Oversized requests are asked to split; exhausted global budget is visible in the Actions summary and retried by later sweeps. Slots are reserved before paid calls, so failures also consume the reservation. Network retries can make up to three API attempts per reviewed project. This is separate from the discovery radar budget, not a limit on refreshing existing GitHub statistics.
- **Cache:** unchanged submissions reuse the report; existing-catalog membership changes invalidate that cache. Maintainer commands can request a fresh check after cooldown. Interrupted/API-error reports retry after cooldown. A low-confidence result is not repeatedly sent to the model until it passes: it waits for changed submission evidence or an explicit maintainer request.
