...
The news shortcut swaps the main result area, not the header or design system. It has local text search, source attribution, newest-first order, and an incremental “show more” control rather than rendering an unbounded archive at once. Unlike the compact GitHub grid, news uses full-width cards grouped by Shanghai calendar date with a time rail, source line, AIHOT score when present, title, summary, and optional original-title excerpt. The layout borrows the reading order of AIHOT's timeline but retains our tokens and radii. A plain click opens our own accessible, dismissible summary dialog with the original-source and AIHOT reading links; modified clicks retain the AIHOT link fallback. The dialog does not claim to display the article body or media, because the official API does not expose those fields; see [news.md](news.md) for the source boundary.
