...
import { evaluateJev, parseScore, reviewDecision, reviewBody } from './jev-client.mjs'
import { createGitHubClient } from './github-client.mjs'
import { emptyState, runRadar, validateState } from './radar.mjs'

const now = new Date('2026-09-22T00:00:00Z')
const score = { jevAbout: 0.95, jevKeep: 'keep', jevKeepConfidence: 0.99 }
const response = { answers: { about: { type: 'noul', noul: 0.95 }, keep: { type: 'choice', choice: 'keep', confidence: 0.99 } } }
const repo = { html_url: 'https://github.com/test/jev-sdk', full_name: 'test/jev-sdk', name: 'jev-sdk',
  default_branch: 'main', owner: { login: 'test' }, description: 'TypeSafe Jev SDK', topics: ['jev', 'sdk'],
  stargazers_count: 3, forks_count: 1, open_issues_count: 0, language: 'TypeScript', created_at: now.toISOString() }
const catalog = () => ({ files: new Map([['github.json', []], ['youtube.json', []]]), rows: [], social: [] })
...
test('review scope includes curated lists, not only direct API integration', () => {
  assert.match(reviewBody({ type: 'github', title: 'awesome', url: repo.html_url }).questions.keep.instructions, /curated awesome lists/)
})
test('missing key never calls network; HTTP errors never echo response bodies', async () => {
  ...
  }), (error) => error.message === 'jev-http-401')
})
test('rate limits back off and retry without logging credentials', async () => {
  let calls = 0; const waits = []
  const result = await evaluateJev('test-only', {}, '', { fetchImpl: async (_, init) => {
...
test('GitHub client bounds retries and rejects alternate origins', async () => {
  const seen = []
  const client = createGitHubClient('test-only', { fetchImpl: async (url, init) => {
    seen.push(url); assert.equal(init.redirect, 'error'); return Response.json({ ok: true })
  } })
...
for (const verdict of ['review', 'drop']) test(`${verdict} is queued with future retry, never published`, async () => {
  const result = await runRadar({ ...options(), review: async () => ({ ...score, jevKeep: verdict }) })
  ...
  assert.ok(Date.parse(result.state.candidates['test/jev-sdk'].retryAt) > now.getTime())
})
...
test('missing README never reaches Jev and cannot enter catalog', async () => {
  let called = false
  const result = await runRadar({ ...options(), api: async (path) => {
...
test('bad persisted state and invalid run limits fail before external calls', async () => {
  ...
  await assert.rejects(() => runRadar({ ...options(), limit: 1000 }), /limit/)
})
...
test('CLI missing-key preflight cannot touch output or load local env', async (t) => {
  ...
  const child = spawnSync(process.execPath, ['scripts/radar.mjs', output], { env, encoding: 'utf8' })
...
test('manual scorer enforces one global paid-review limit across all sources', async (t) => {
  ...
  for (const file of ['score-sources.ts', 'catalog.mjs', 'jev-client.mjs']) copyFileSync(`scripts/${file}`, join(root, 'scripts', file))
...
test('older unreviewed candidates do not starve behind alphabetically earlier new arrivals', async () => {
  ...
  const visited = []
  await runRadar({ ...options(), state, queries: [], limit: 1, api: async (path) => {
...
test('persisted timestamps must be ISO strings, not Date.parse-coercible numbers', () => {
  const state = emptyState()
  state.candidates['test/bad'] = { status: 'pending', discoveredAt: 0, attempts: 0 }
